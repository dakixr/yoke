...
def test_skill_activation_payload_omits_context_instructions(
    ...
    """The tool result does not repeat instructions added as context."""
    skill_root = tmp_path / "compact-skill"
    skill_root.mkdir()
    skill_md = skill_root / "SKILL.md"
    skill_md.write_text("large canonical instructions", encoding="utf-8")
    ...
    assert activation.active_payload() == [
        {
            "name": "compact-skill",
            "description": "Compact skill.",
            "source_path": str(skill_md),
    ...
    assert "large canonical instructions" in (
        render_active_skill_message(activation.activated_skills[0]).plain_text_content
        or ""
    )
