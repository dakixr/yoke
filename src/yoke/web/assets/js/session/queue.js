...
export function QueueEditor({ sessionID, queue, unavailable = false }) {
  ...
  return html`<section class="queue-editor" aria-label="Pending work">
    ...
    <div class="queue-editor__items">
      ${queue.items.map((item, index) => html`<${QueueItem} key=${item.id} sessionID=${sessionID} queue=${queue} item=${item} index=${index} unavailable=${unavailable} />`)}
    </div>
  </section>`;
}

function QueueItem({ sessionID, queue, item, index, unavailable }) {
  ...
  return html`<div class=${`queue-item ${item.paused ? "is-paused" : ""}`}>
    ...
    <div class="queue-item__body">
      <textarea rows="1" value=${text} readOnly=${unavailable} aria-label="Queued prompt" onInput=${(event) => setText(event.currentTarget.value)} onBlur=${() => !unavailable && text !== item.prompt?.text && save()}></textarea>
      ...
      <div class="queue-item__controls">
        <select value=${item.delivery} disabled=${unavailable} aria-label="Queue delivery" onChange=${(event) => controller.patchQueue(sessionID, [{ op: "setDelivery", id: item.id, delivery: event.currentTarget.value }])}>
          <option value="steer">Steer now</option><option value="queue">Queue next</option>
        </select>
        <button disabled=${unavailable} onClick=${() => controller.patchQueue(sessionID, [{ op: "setPaused", id: item.id, paused: !item.paused }])}>${item.paused ? "Resume" : "Pause"}</button>
        <button disabled=${unavailable || index === 0} title="Move to start" onClick=${() => controller.patchQueue(sessionID, [{ op: "moveToStart", id: item.id }])}>To start</button>
        <button disabled=${unavailable || index === 0} aria-label="Move up" title="Move up" onClick=${moveUp}>↑</button>
        <button disabled=${unavailable || index === queue.items.length - 1} aria-label="Move down" title="Move down" onClick=${moveDown}>↓</button>
