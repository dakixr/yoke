...
`deepseek-flash` and `deepseek-v4-flash` at 400,000 tokens.
...
uses the Responses path; GLM-5.3-Flash, DeepSeek V4.1 Flash (`deepseek-flash`),
and DeepSeek V4 Flash use OpenAI-compatible chat completions. DeepSeek V4.1
Flash supports image inputs and `low`, `high`, and `max` reasoning efforts
(default: `high`). Yoke sends `x-opencode-session` with one stable value per
