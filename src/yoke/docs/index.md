...
The Codex providers mirror Codex CLI's session-affinity behavior: they derive a stable prompt cache key from the yoke session id, so the same cache key survives provider reconstruction and `yoke resume`. New and forked sessions receive distinct keys. SDK-created providers without a session id keep one generated key for the provider instance. The providers also capture `x-codex-turn-state` metadata from the server and replay that sticky-routing token only on follow-up model requests within the same logical user turn; a new user turn clears it. The WebSockets variant requests `all_turns` reasoning context, retains encrypted Responses output items in memory, and probes cached sockets before reuse so long-running tool gaps do not keep sending on a stale connection. When a follow-up request is a prefix extension of the prior logical transcript on the same account, it sends only the new input delta with `previous_response_id`; after a reasoning-backed function call, that delta contains the function result without redundantly replaying the call. If the connection-local response anchor is stale, the provider retries by replaying its retained encrypted items plus the new input. Forked provider instances carry that replay state but intentionally drop the socket-local response id. If automatic account rotation selects a different account, the WebSockets provider starts a fresh Codex server session from visible full context because response and encrypted replay state remain account-scoped.
