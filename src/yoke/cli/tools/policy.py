...
def load_config_file(path: Path) -> LoadedWorkspaceConfig:
    """load_config_file."""
    if not path.is_file():
        return LoadedWorkspaceConfig(path=None, config=YokeConfig())
    try:
        payload = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise ValueError(f"Could not read yoke config file `{path}`: {exc}") from exc
    try:
        config = YokeConfig.model_validate_json(payload)
...
def default_yoke_config() -> YokeConfig:
    """default_yoke_config."""
    return YokeConfig(
...
def merge_configs(*configs: YokeConfig) -> YokeConfig:
    """merge_configs."""
...
def is_tool_allowed(name: str, config: YokeConfig) -> bool:
    """is_tool_allowed."""
...
def is_loaded_tool_allowed(tool: LoadedTool, config: YokeConfig) -> bool:
...
def unmatched_tool_patterns(
    config: YokeConfig, known_tool_names: set[str]
) -> list[str]:
...
def _normalize_reasoning_effort(value: str) -> str:
    ...
    if normalized not in {"none", "low", "medium", "high", "xhigh", "max"}:
        raise ValueError("Expected one of none, low, medium, high, xhigh, or max.")
    return normalized
