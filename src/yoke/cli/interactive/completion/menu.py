    ...
    def _completion_fragments(
        ...
        *,
        ...
        fragments.append(
            (
                command_style,
                " " * max(0, command_width - command_fragment_width),
            )
        )
        if meta_width <= 0:
            return fragments
...
def register_completion_menu_key_bindings(key_bindings: KeyBindings) -> None:
    ...
    def _move_cursor_left(event: KeyPressEvent) -> None:
        ...
        _move_cursor_across_lines(buffer, -event.arg)
...
def _move_cursor_across_lines(buffer, offset: int) -> None:
    """Move horizontally through the whole buffer, including newlines."""
    buffer.cursor_position = max(
        0,
        min(len(buffer.text), buffer.cursor_position + offset),
    )


def selected_completion(complete_state) -> object | None:
