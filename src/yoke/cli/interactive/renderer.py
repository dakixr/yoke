...
from yoke.cli.interactive.toolbar import (
...
from yoke.cli.render import format_tool_result_error
...
class PromptToolkitLiveRenderer:
    ...
    def __init__(
        ...
        *,
        ...
        self._emit_warning = emit_warning or emit_notice
    ...
    def handle_event(self, event: str, payload: dict[str, object]) -> None:  # noqa: C901
        """Handle one runtime event."""
        self._record_tool_event_if_needed(event, payload)
        if is_provider_event(event):
            self._emit_warning(format_provider_event(event, payload))
            self._set_status_text(provider_status_for_event(event))
            return
