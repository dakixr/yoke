...
def _legacy_byte_fallback(error: UnicodeError) -> tuple[str, int]:
    if not isinstance(error, UnicodeDecodeError):
        raise error
    return error.object[error.start : error.end].decode(
        "cp1252", errors="replace"
    ), error.end


codecs.register_error("yoke-command-text", _legacy_byte_fallback)


def command_decoder() -> codecs.IncrementalDecoder:
    """Keep split UTF-8 characters intact while accepting legacy terminal bytes."""
    return codecs.getincrementaldecoder("utf-8")(errors="yoke-command-text")
...
class _OutputRecord:
    ...
    def decoded(self, *, include_previous: bool, exact: bool = False) -> str:
    ...
    def page(
        ...
        records = (
            ...
            if after_seq >= self.latest_seq
                ...
                islice(
                    (record for record in self._retained if record.seq > after_seq),
                    limit,
                )
            )
        )
        return CommandProcessOutputPage(
            chunks=_page_chunks(records, after_seq=after_seq, exact=exact),
...
class CompletedCommandProcess:
    ...
    def output_page(
        ...
        records = (
            ()
            if after_seq >= self.snapshot.latest_output_seq
            else tuple(
                islice(
                    (
                        record
...
def _page_chunks(
    records: tuple[_OutputRecord, ...], *, after_seq: int, exact: bool = False
